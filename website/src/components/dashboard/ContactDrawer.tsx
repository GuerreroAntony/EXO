            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <section>
                ...
                {contato.notas.length > 0 ? (
                  <div className="space-y-3">
                    {contato.notas.map((nota) => (
                      ...
                      >
                        <p className="text-sm text-white/70 leading-relaxed">{nota.texto}</p>
                  ...
                  <p className="text-sm text-white/20">Nenhuma nota registrada.</p>
                )}
                ...
                <textarea
                  ...
                  rows={3}
                  className="mt-3 w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/20 transition-colors resize-none"
                />
              </section>
              ...
              <section>
                ...
                {contato.timeline.length > 0 ? (
                  <div className="relative space-y-0">
                    ...
                    {contato.timeline.map((item) => (
                      <div key={item.id} className="relative flex gap-3 py-3">
                        {/* Icon circle */}
                        <div className="relative z-10 flex items-center justify-center w-[23px] h-[23px] rounded-full bg-white/[0.06] border border-white/[0.10] text-white/50 shrink-0">
                          {timelineIcon[item.tipo]}
                        </div>

                        {/* Content */}
                        <div className="min-w-0 flex-1">
                          <p className="text-sm text-white/70 leading-snug">{item.resumo}</p>
                          <div className="flex flex-wrap items-center gap-2 mt-1.5">
                            <span className="text-[11px] text-white/30">
                              {formatDate(item.data)}
                            </span>
                            <span className="text-[11px] text-white/20">&middot;</span>
                            <span className="text-[11px] text-white/30">{item.agente}</span>
                  ...
                  <p className="text-sm text-white/20">Nenhum evento na timeline.</p>
                )}
              </section>
            </div>
          </motion.div>
